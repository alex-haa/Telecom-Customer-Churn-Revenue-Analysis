-- =====================================================
-- TELECOM CUSTOMER CHURN & REVENUE ANALYSIS - POSTGRESQL
-- SQL Queries for Customer Lifetime Value, Churn Drivers, and Pricing Analysis
-- =====================================================
...
WITH customer_clv AS (
    SELECT 
        ...
        CASE 
            WHEN TotalCharges IS NOT NULL AND TotalCharges > 0 
            THEN TotalCharges
...
-- Customer cohort analysis (simulated)
WITH customer_cohorts AS (
    SELECT 
        ...
        MonthlyCharges,
        Churn,
        -- Simulate cohort based on tenure
        CASE 
            WHEN tenure <= 12 THEN '0-12 months'
            WHEN tenure <= 24 THEN '13-24 months'
            WHEN tenure <= 36 THEN '25-36 months'
...
ORDER BY 
    CASE 
        WHEN cohort_group = '0-12 months' THEN 1
        WHEN cohort_group = '13-24 months' THEN 2
        WHEN cohort_group = '25-36 months' THEN 3
        WHEN cohort_group = '37-48 months' THEN 4
        WHEN cohort_group = '49-60 months' THEN 5
        ELSE 6
    END;
...
WITH revenue_forecast AS (
    SELECT 
        ...
        -- Projected churn reduction scenarios
        ROUND(COUNT(*) * AVG(MonthlyCharges) * 12, 2) AS current_annual_revenue,
        ROUND(COUNT(*) * AVG(MonthlyCharges) * 12 * 0.8, 2) AS revenue_with_20pct_churn_reduction,
        ROUND(COUNT(*) * AVG(MonthlyCharges) * 12 * 0.6, 2) AS revenue_with_40pct_churn_reduction
    FROM telecom_customers
    GROUP BY Contract, InternetService
)
SELECT 
    ...
    revenue_with_20pct_churn_reduction,
    revenue_with_40pct_churn_reduction,
    ROUND(revenue_with_20pct_churn_reduction - current_annual_revenue, 2) AS revenue_gain_20pct,
    ROUND(revenue_with_40pct_churn_reduction - current_annual_revenue, 2) AS revenue_gain_40pct
FROM revenue_forecast
ORDER BY current_annual_revenue DESC;
